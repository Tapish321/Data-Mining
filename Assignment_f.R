Homework_1 <- function(){
  ...
  #2D Scatter Plots
  plot(iris[,1:4],col = rainbow(1), pch = 20)
  ...
  filled.contour(z =ir_matrix,plot.title = ("Iris Feature Visualization"),color = terrain.colors)
 ...
 # library(c("car","rgl","scatterplot3d"))
  scatterplot3d::scatterplot3d(x = iris[,1], y = iris$Petal.Length, z = iris$sepal.Length, grid=TRUE,color = colors,pch = 20,angle = 40)+grid(nx=NULL)
  ...
  scatterplot3d::scatterplot3d(iris[,c(1,2,4)],main= "3D Scatter Plot",xlab = "Sepal Length in cm",ylab="Petal Length in cm",zlab = "Petal
                               Width in cm",pch=16,color=colors,angle = 50, main)+grid(nx=NULL)
  ...
  qplot(iris2$Sepal.Length[iris2$Species== 1], geom="histogram",bins = 10,colour = 'red', binwidth = 0.10, xlab = "Sepal length with setosa")
  qplot(iris2$Sepal.Length[iris2$Species== 2], geom="histogram",bins = 10,colour = 'red', binwidth = 0.1, xlab = "Sepal length with versicolor")
  qplot(iris2$Sepal.Length[iris2$Species== 3], geom="histogram",bins = 10,colour = 'red', binwidth = 0.10, xlab = "Sepal length with virginica")
  qplot(iris2$Sepal.Width[iris2$Species== 1], geom="histogram",bins = 10,colour = 'red', binwidth = 0.10, xlab = "Sepal Width with setosa")
  qplot(iris2$Sepal.Width[iris2$Species== 2], geom="histogram",bins = 10,colour = 'red', binwidth = 0.1, xlab = "Sepal Width with versicolor")
  qplot(iris2$Sepal.Width[iris2$Species== 3], geom="histogram",bins = 10,colour = 'red',  xlab = "Sepal Width with virginica")
  qplot(iris2$Petal.Length[iris2$Species== 1], geom="histogram",bins = 10,colour = 'red',  xlab = "Sepal length with setosa")
  qplot(iris2$Petal.Length[iris2$Species== 2], geom="histogram",bins = 10,colour = 'red',  xlab = "Sepal length with versicolor")
  qplot(iris2$Petal.Length[iris2$Species== 3], geom="histogram",bins = 10,colour = 'red', xlab = "Sepal length with virginica")
  qplot(iris2$Petal.Width[iris2$Species== 1], geom="histogram",bins = 10,colour = 'red', xlab = "Sepal Width with setosa")
  qplot(iris2$Petal.Width[iris2$Species== 2], geom="histogram",bins = 10,colour = 'red',  xlab = "Sepal Width with versicolor")
  qplot(iris2$Petal.Width[iris2$Species== 3], geom="histogram",bins = 10,color = 'red',  xlab = "Sepal Width with virginica")
  #Box Plots
  #boxplot(Sepal.Length~Species,color=c("blue","green","red"), data = iris)
  BP_SL <- ggplot(iris,aes(iris[,1:4],Species))+geom_boxplot()+ scale_y_continuous("Sepal Length in cm", breaks= seq(0,30, by=.5))+
  ...
  BP_SW
  BP_PW
  boxplot(iris2[1:4],iris2[iris2$Species==1])
  ...
  mahalabboni_dist=function(a,b,m){
  ...
  plot(Mdist_1,  type = "l")
  plot(Mdist_1,pch=30,main = "Minkowski Distances plot at r = 1" ,xlab = "observations")+grid(col = "lightgray", lty = "dotted") 
                + lines(Mdist_1,pch = 16)
  
  #lines(x[order(x)], r1[order(x)],  pch=16)
  plot(Mdist_2,  type = "l", pch = 20)
  plot(Mdist_2,pch=30,main = "Minkowski Distances plot at r = 2",  xlab = "observations",ylab = "Length")+grid(col = "lightgray", lty = "dotted",
                                                                                             lwd = par("lwd")) + lines(Mdist_2,pch = 16)
  
  plot(Mdist_3,  type = "l")
  plot(Mdist_3,pch=30,main = "Minkowski Distances plot at r = 100",xlab = "observations",ylab = "Length")+grid(col = "lightgray", lwd = par("lwd")) + lines(Mdist_3,pch = 16)
  
  
  
  plot(mahaDist,  type = "l")
  plot(mahaDist,pch=30,main = "Mahalanobis  Distances plot",xlab = "observations", ylab = "Length")+grid(col = "lightgray", lty = "dotted",
  ...
  normalize_iris <- function(x) {
    for(n in 1:ncol(x)){
      mean = sum(x[n])/ nrow(x[n])
      
      x[n] = (x[n]-mean)/(max(x[n])-min(x[n]))
    }
    
    return (x)
  }
  he <-(normalize_iris(iris[1:4]))
